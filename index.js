...
// Prev Post (for navigating back to full_post page)
let prevPost = "Null"
...
app.get("/", (req, res) => {
  res.render(__dirname + "/views/index.ejs", { posts: posts, colorMode: colorMode });
});

app.get("/all", (req, res) => {
    res.render(__dirname + "/views/all_posts.ejs", { posts: posts, colorMode: colorMode });
});

app.get("/tech", (req, res) => {
    res.render(__dirname + "/views/tech.ejs", { posts: posts, colorMode: colorMode });
});

app.get("/science", (req, res) => {
    res.render(__dirname + "/views/science.ejs", { posts: posts, colorMode: colorMode });
});

app.get("/nature", (req, res) => {
    res.render(__dirname + "/views/nature.ejs", { posts: posts, colorMode: colorMode });
});

app.get("/create", (req, res) => {
    res.render(__dirname + "/views/create.ejs", { posts: posts, colorMode: colorMode });
});

app.get("/full_post", (req, res) => {
    console.log("full_post get called")
    console.log(JSON.stringify(prevPost))
    if(prevPost == "Null"){
      prevPost = posts[posts.length - 1];
    }
    res.render(__dirname + "/views/full_post.ejs", { post: prevPost, colorMode: colorMode })
});

app.post("/full_post", (req, res) => {
    let post = posts.find(post => post.uid == req.body.uid);
    prevPost = post;
    res.render(__dirname + "/views/full_post.ejs", { post: post, colorMode: colorMode });
});
...
app.post("/create", (req, res) => {
    ...
    let date = new Date()
    newPost.date = `${date.toLocaleString('default', { month: 'short' })} ${date.getDate()} ${date.getFullYear()}`
    ...
    console.log(newPost);
    posts.push(newPost);
    res.render(__dirname + "/views/create.ejs", { posts: posts, submitted: true, colorMode: colorMode });
});

app.post("/colormode", (req, res) => {
  colorMode = req.body.colorMode
  console.log("Color Mode Set to " + req.body.colorMode)
  ...
  switch(req.body.location){
    ...
    case "None":
      if(req.body.postuid){
        prevPost = posts.find(post => post.uid == req.body.postuid);
        
        res.redirect("/full_post");
      } else {
        res.redirect("/");
      }
      break;
